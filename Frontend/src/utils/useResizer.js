...
export const useResizer = (className, urls, delta) => {
  ...
  const resizeEnd = async () => {
    console.debug('[DEBUG]', timeoutRef, rtime, timeout, imageProps, className, urls, delta)
    ...
    if (new Date() - rtime < delta) {
      ...
      if (typeof photo === 'object') {
        ...
        if (width !== imageProps.w || height !== imageProps.h || dpr !== imageProps.dpr) {
          try {
            await Promise.all(urls.map((url) => preloader(imgUrlBuilder(url, width, height, dpr))))
          } catch (e) {
