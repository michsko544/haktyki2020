...
export const settingsFormValidation = Yup.object()
  .shape({
    user: Yup.string()
      .matches(/^[A-Za-zżźćńółęąśŻŹĆĄŚĘŁÓŃ ]*$/, 'Pole nie może zawierać znaków specjalnych, ani cyfr')
      .matches(/^[A-zżźćńółęąśŻŹĆĄŚĘŁÓŃ]+\s+[A-zżźćńółęąśŻŹĆĄŚĘŁÓŃ]+ ?$/, 'Podaj dwa wyrazy')
    ...
    blik: Yup.string()
      ...
      .matches(/^[0-9]{3}[ ]{0,1}[0-9]{3}[ ]{0,1}[0-9]{3} ?$/, 'Podaj poprawny, 9-cyfrowy numer telefonu'),
    account: Yup.string()
    ...
    swift: Yup.string()
      .when('account', {
        is: (account) =>
          account
            ? account.length > 1 &&
              /^[A-Z]*$/.test((account[0] + account[1]).toString().toUpperCase()) &&
      ...
      .matches(/^[A-Za-z0-9]*$/, 'Pole musi zawierać tylko duże litery i/lub cyfry'),
  })
  .test({
    name: 'at-least-one-number',
    test: function (value) {
      return value.account !== undefined || value.blik !== undefined
        ? true
        : this.createError({ message: 'Musisz podać conajmniej jeden numer (konta/telefonu)', path: 'account' })
    },
  })
